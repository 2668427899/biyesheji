...
@main_bp.route("/pythons53e51x2/cal/<tableName>/<columnName>", methods=['GET'])
def pythons53e51x2_cal(tableName, columnName):
    '''
    计算规则接口
    '''
...
@main_bp.route("/pythons53e51x2/file/download", methods=['GET'])
def pythons53e51x2_file_download():
    '''
    '''
    if request.method == 'GET':
...
@main_bp.route("/pythons53e51x2/file/upload", methods=['POST'])
def pythons53e51x2_file_upload():
    '''
    '''
...
@main_bp.route("/pythons53e51x2/follow/<tableName>/<columnName>/<level>/<parent>", methods=['GET'])
def pythons53e51x2_follow_level(tableName, columnName, level,parent):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/pythons53e51x2/follow/<tableName>/<columnName>", methods=['GET'])
def pythons53e51x2_follow(tableName, columnName):
    '''
...
@main_bp.route("/pythons53e51x2/location", methods=['GET'])
def pythons53e51x2_location():
    '''
...
@main_bp.route("/pythons53e51x2/matchFace", methods=['GET'])
def pythons53e51x2_matchface():
    '''
    baidubce百度人脸识别
    '''
    if request.method == 'GET':
...
@main_bp.route("/pythons53e51x2/option/<tableName>/<columnName>", methods=['GET'])
def pythons53e51x2_option(tableName, columnName):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/pythons53e51x2/remind/<tableName>/<columnName>/<type>", methods=['GET'])  #
def pythons53e51x2_remind(tableName, columnName,type):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/pythons53e51x2/sh/<tableName>", methods=['POST'])
def pythons53e51x2_sh(tableName):
    '''
    '''
    if request.method == 'POST':
        ...
        logging.warning("pythons53e51x2_sh.res=====>{}".format(res))
        return jsonify(msg)

@main_bp.route("/pythons53e51x2/upload/<fileName>", methods=['GET'])
def pythons53e51x2_upload(fileName):
    '''
    上传接口
    '''
    if request.method == 'GET':
...
@main_bp.route("/pythons53e51x2/group/<tableName>/<columnName>", methods=['GET'])
...
@main_bp.route("/pythons53e51x2/value/<tableName>/<xColumnName>/<yColumnName>", methods=['GET'])
...
# 爬取数据
@main_bp.route("/pythons53e51x2/spider/<tableName>", methods=['GET'])
def schema_spider(tableName):
    msg = {"code": normal_code, "msg": "success", "data": []}

    # Linux
    # cmd = "cd /yykj/python/9999/spider0e5x7iq6 && scrapy crawl "+tableName+"Spider -a databaseName=pythons53e51x2"
    # Windows
    cmd = "cd D:\\biyesheji\\spider && scrapy crawl " + tableName + "Spider -a databaseName=pythons53e51x2"
    os.system(cmd)
